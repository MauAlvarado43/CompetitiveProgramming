...
int busqueda(vector<int> t_from, vector<int> t_to, int num, int t_edges) {
    int nodos = 1;
    for(int i = 0; i < t_edges; i++) {
        if(num == t_to[i]) {
            nodos+=busqueda(t_from, t_to, t_from[i], t_edges);
        }
    }
    return nodos;
}
...
int evenForest(int t_nodes, int t_edges, vector<int> t_from, vector<int> t_to) {
    int cortes = 0;
    for(int i = 0; i < t_edges; i++) {
        if(busqueda(t_from, t_to, t_from[i], t_edges) % 2 == 0)
            cortes++;
    }
    return cortes;
}
